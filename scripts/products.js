
 export const products = [{
  id:"id1",
  image:"images/men's-sneakers-khaki.jpg",
  name:"Men's Breathable Lace up Sneakers - Khaki",
  priceCent:1193,
  oldpriceCent:1272,
  category:"fashion",
  rating:{stars:4.5, count:137},
  unit:6,
},

{
  id:"id2",
  image:"images/men's-printed-t-shirt.jpg",
  name:"Men's Printed T-Shirt 4 in 1 Black/White.",
  priceCent:2206,
  oldpriceCent:2551,
  category:"fashion",
  rating:{stars:4.0, count:108},
  unit:4,
},

{
  id:"id3",
  image:"images/ladies-red-hand-bag.jpg", 
  name:"Ladies Shoulder Hand Bag - Black & White",
  priceCent:768,
  oldpriceCent:813,
  category:"fashion",
  rating:{stars:4.0, count:47},
  unit:2,
},

{
  id:"id4",
  image:"images/acoustic-guitar.jpg",
  name:"Acoustic Box Guitar-Black 38",
  priceCent:3552,
  oldpriceCent:3652,
  category:"appliances",
  rating:{
  stars:4.5, count:22},
  unit:2,
},

{
  id:"id5",
  image:"images/molten-basket-ball.jpg",
  name:"Molten NBA Pro Basket Ball",
  priceCent:1981,
  oldpriceCent:2091,
  category:"sports", 
  rating:{stars:4.5, count:29},
  unit:2,
  },

{
  id:"id6", 
  image:"images/learners-keyboard-piano.jpg", 
  name:"54 Keys Learners XY-813 Keyboard-piano",
  priceCent:5186,
  oldpriceCent:5386,
  category:"appliances",
  rating:{stars:4.5, count:97},
  unit:3,
},

{
  id:"id7", 
  image:"images/keyboard-sustain-pedal.jpg", 
  name:"Sustain Pedal Keyboard piano",
  priceCent:868,
  oldpriceCent:911,
  category:"appliances",
  rating:{stars:4.5, count:5},
  unit:2,
},

{
  id:"id8", 
  image:"images/black-leather-bag.jpg", 
  name:"Pu Women leader Hand Bag-Black",
  priceCent:506,
  oldpriceCent:606,
  category:"fashion",
  rating:{stars:4.0, count:31},
  unit:3,
},

{
  id:"id9", 
  image:"images/wilson-racket.jpg", 
  name:"Wilson Lawn Tennis Racket",
  priceCent:2095,
  oldpriceCent:2195,
  category:"sports",
  rating:{stars:4.5, count:34},
  unit:2,
},

{
  id:"id10", 
  image:"images/2-racket.jpg", 
  name:"Basket Ball Bat Racket Outdoor Sports",
  priceCent:2204,
  oldpriceCent:2304,
  category:"sports",
  rating:{stars:4.5, count:2},
  unit:3,
},

{
  id:"id11", 
  image:"images/celebrety-outfit.jpeg", 
  name:"Celebrety Outfit Wears ",
  priceCent:945,
  oldpriceCent:102,
  category:"fashion",
  rating:{stars:4.5, count:9},
  unit:2,
},

{
  id:"id12", 
  image:"images/blue-shoe.jpg", 
  name:"Unisex Blue Shoe for Out door Fits",
  priceCent:567,
  oldpriceCent:589,
  category:"fashion",
  rating:{stars:4.5, count:3},
  unit:4,
},

{
  id:"id13", 
  image:"images/men's-sleeveless.jpg", 
  name:"Men's Sleeveless 3 in 1 Shirts ",
  priceCent:2234,
  oldpriceCent:2355,
  category:"fashion",
  rating:{stars:4.0, count:31},
  unit:5,
},

{
  id:"id14", 
  image:"images/suit-trousers.jpg", 
  name:"Men's Official Suit Trousers",
  priceCent:555,
  oldpriceCent:567,
  category:"fashion",
  rating:{stars:4.5, count:23},
  unit:5,
},

{
  id:"id15", 
  image:"images/baby-toys.jpg", 
  name:"Baby Table Toys  ",
  priceCent:123,
  oldpriceCent:132,
  category:"sports",
  rating:{stars:4.0, count:11},
  unit:5,
},

{
  id:"id16", 
  image:"images/women's-standard-kit.jpg", 
  name:"Women's Casual Standard Kit ",
  priceCent:434,
  oldpriceCent:455,
  category:"fashion",
  rating:{stars:4.5, count:9},
  unit:3,
},

{
  id:"id17", 
  image:"images/summer-cover.jpg", 
  name:"Summer Cover For Women -Yellow",
  priceCent:477,
  oldpriceCent:588,
  category:"fashion",
  rating:{stars:4.0, count:4},
  unit:4,
},

{
  id:"id18", 
  image:"images/white-sneakers.jpg", 
  name:"Men's Casual Sneakers-White",
  priceCent:9452,
  oldpriceCent:1021,
  category:"fashion",
  rating:{stars:4.5, count:322},
  unit:2,
},

{
  id:"id19", 
  image:"images/unisex-white-sneakers.jpg", 
  name:"Unisex Beautiful White Sneakers ",
  priceCent:9452,
  oldpriceCent:1021,
  category:"fashion",
  rating:{stars:4.5, count:322},
  unit:4
}
];


