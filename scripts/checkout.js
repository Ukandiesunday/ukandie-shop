  ...
  js-container-grid-${matchingProduct.id}">
    <div class="product-container-grid">
      <div class="product-image-container">
        ...
        }" alt="khaki-shoe" width="100" height="100" class="product-image">
      </div>
      <div class="product-description-container">
        <div class="product-name">${matchingProduct.name}</div>
        <div class="product-price">$${calculateMoney(
          matchingProduct.priceCent
        )}</div>
        <div class = "quantity-price">$${
          (cartItem.quantity * matchingProduct.priceCent) / 100
        }</div>
        <div class="product-oldprice">$${calculateMoney(
          matchingProduct.oldpriceCent
        )}</div>
    ...
    <div class="update-container">
      ...
      <button class="remove-button  js-remove-button" data-product-id = "${
        matchingProduct.id
      }"> Remove   <i class="fa-solid fa-trash"></i></button>
      </a>
    </div>
  </div>
 `;
...
document.querySelector(
  ".js-cart-summary"
).innerHTML = `<h2>Cart Summary: $${calcTotalBill(amount)}</h2>`;

document.querySelector(".cart-total-quantity").innerHTML = checkoutCartQty;
//To filter out items
document.querySelectorAll(".js-remove-button").forEach((removeBtn) => {
...
copyRight.innerHTML = `<span>&copy;Copyright</span> ${year}. All rights reserved. Coded by Ukandie Sunday`;
