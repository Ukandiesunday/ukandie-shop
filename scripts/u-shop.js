...
function renderHTML() {
  ...
  <div class="img-wrapper">
    ...
    <div class="product-rating-container">
      <img src="images/icons/product-rating-${
        product.rating.stars * 10
      }.png" alt="ratings" class="rating-stars">
    ...
    <div class="select-product-quantity">
     ...
     <button class="decrease-btn js-decrease-btn-${product.id}">-</button> 
      <div class="update" id="update-${product.id}">${
    ...
    <button class="add-to-cart-btn js-add-to-cart-btn"  const data-product-id = ${
      product.id
    }>Add to cart</button>
  </div>
 `;
...
btns.forEach((button) => {
  button.addEventListener("click", () => {
    ...
    document.querySelector(`.js-btns-container-${productId}`).style.opacity =
      "1";

    // To decrease cartQty via  minus (-) button
    const decreaseBtn = document.querySelectorAll(
      `.js-decrease-btn-${productId}`
    );
    ...
    document
      .querySelectorAll(`.js-increase-btn-${productId}`)
      .forEach((increaseBtn) => {
...
function addMessage(popUpMessage) {
  ...
  !messages.classList.contains("popUp") && messages.classList.add("popUp");
  setTimeout(() => {
